...
function bindEvents() {
  ...
  elements.pollTableBody?.addEventListener("toggle", (event) => {
    const details = event.target;
    if (!(details instanceof HTMLDetailsElement)) {
      return;
    }

    const districtCode = details.dataset.districtCode;
    if (!districtCode) {
      return;
    }

    if (details.open) {
      state.expandedDistricts.add(districtCode);
    } else {
      state.expandedDistricts.delete(districtCode);
    }
  }, true);
...
function renderTurnoutChart(filteredPolls, selectedDistrict) {
  ...
  const districts = electionData.districts
    ...
    .slice(0, electionData.districts.length);

  if (elements.turnoutChartTitle) {
    elements.turnoutChartTitle.textContent = "All constituencies by turnout";
  }
...
function renderTable(filteredPolls) {
  ...
  if (elements.tableNote) {
    const districtCount = new Set(filteredPolls.map((poll) => poll.districtCode)).size;
    elements.tableNote.textContent =
      `${numberFormatter.format(districtCount)} districts • ${numberFormatter.format(filteredPolls.length)} poll rows`;
  }
  ...
  if (!filteredPolls.length) {
    elements.pollTableBody.innerHTML =
      '<div class="empty-state">No polls match the current filters.</div>';
    return;
  }
  ...
  state.expandedDistricts = new Set(
    [...state.expandedDistricts].filter((districtCode) => groupedPolls.has(districtCode)),
  );

  const orderedDistricts = electionData.districts.filter((district) => groupedPolls.has(district.code));

  elements.pollTableBody.innerHTML = orderedDistricts
    .map((district) => {
      const districtPolls = groupedPolls.get(district.code) || [];
      const summary = summarizePolls(districtPolls);
      const winner = aggregateCandidateVotes(districtPolls).sort((left, right) => right.votes - left.votes)[0];
      return `
        <details class="district-item" data-district-code="${escapeHtml(district.code)}" ${
          state.expandedDistricts.has(district.code) ? "open" : ""
        }>
          <summary class="district-summary">
            <div class="district-summary-main">
              <strong class="district-summary-name">${escapeHtml(district.displayName)}</strong>
              <span class="district-summary-subtitle">Click to view polls in this district</span>
            </div>
            <div class="district-summary-stats">
              ${renderDistrictSummaryStat("Polls", numberFormatter.format(districtPolls.length))}
              ${renderDistrictSummaryStat("Votes", numberFormatter.format(summary.totalVotes))}
              ${renderDistrictSummaryStat(
                "Turnout",
                typeof summary.turnoutRate === "number" ? percentFormatter.format(summary.turnoutRate) : "n/a",
              )}
              ${renderDistrictSummaryStat(
                "Leader",
                winner ? escapeHtml(formatWinnerDisplay(winner)) : "n/a",
              )}
            </div>
          </summary>
          <div class="district-polls">
            <div class="district-poll-list">
              ${districtPolls
                .map(
                  (poll) => `
                    <article class="district-poll-card">
                      <div class="district-poll-card-head">
                        <div>
                          <strong>Poll ${escapeHtml(poll.pollCode)}</strong>
                          <span>${escapeHtml(poll.pollType)}</span>
                        </div>
                        <span class="district-poll-turnout">${typeof poll.turnoutRate === "number" ? percentFormatter.format(poll.turnoutRate) : "n/a"}</span>
                      </div>
                      <p class="district-poll-location">${escapeHtml(poll.location)}</p>
                      <div class="district-poll-meta">
                        ${renderPollMetaItem("Electors", poll.electors ? numberFormatter.format(poll.electors) : "n/a")}
                        ${renderPollMetaItem("Votes", numberFormatter.format(poll.totalVotes))}
                        ${renderPollMetaItem("Leading candidate", escapeHtml(formatWinnerDisplay(poll.winner)))}
                      </div>
                    </article>
                  `,
                )
...
function renderDistrictSummaryStat(label, value) {
  return `
    <div class="district-summary-stat">
      <span>${label}</span>
      <strong>${value}</strong>
    </div>
  `;
...
function renderPollMetaItem(label, value) {
  return `
    <div class="district-poll-meta-item">
      <span>${label}</span>
      <strong>${value}</strong>
    </div>
  `;
