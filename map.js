...
function getFeatureStyle(record) {
  ...
  return {
    ...
    fillOpacity: record?.hasExactPollMatch ? (isSelected ? 0.9 : isDistrictFocus ? 0.74 : 0.64) : 0,
    opacity: 1,
  };
}
...
function handleFullscreenChange() {
  const panel = elements.mapPanel;
  const isFullscreen = Boolean(panel && document.fullscreenElement === panel);
  panel?.classList.toggle("is-fullscreen", isFullscreen);
  updateFullscreenButton();

  window.setTimeout(() => {
    state.leafletMap?.invalidateSize();
    fitMapToRecords(state.visibleRecords);
  }, 120);
}
...
function updateFullscreenButton() {
  ...
  if (!document.fullscreenEnabled) {
    elements.mapFullscreenButton.hidden = true;
    return;
  }

  const isFullscreen = Boolean(elements.mapPanel && document.fullscreenElement === elements.mapPanel);
  elements.mapFullscreenButton.hidden = false;
  elements.mapFullscreenButton.textContent = isFullscreen ? "Exit full screen" : "Full screen";
  elements.mapFullscreenButton.setAttribute("aria-pressed", String(isFullscreen));
}
  ...
  if (state.currentElection === "2024") {
    return {
      ...
      items: [
        ...
        { label: "Other winner", color: "#6e7687" },
        {
          label: "No exact poll match",
          color: "transparent",
        },
